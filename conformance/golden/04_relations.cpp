...
std::string applyPreloads_Suscripcion(const std::string& arr, const std::string& req) {
    std::string pre = getQueryParam(req, "_preload");
    if (pre.empty()) return arr;
    std::string preKey = "," + pre + ",";
    std::vector<std::string> objs = __splitJsonObjects(arr);
    if (preKey.find(",plan,") != std::string::npos) {
        std::map<std::string, std::string> rel;
        std::vector<std::string> robjs = __splitJsonObjects(Plan::getAllAsJSON(""));
        for (auto& ro : robjs) rel[getJSONVal(ro, "id")] = ro;
        for (auto& o : objs) {
            std::string fk = getJSONVal(o, "plan");
            std::string emb = rel.count(fk) ? rel[fk] : std::string("null");
            size_t lb = o.rfind('}');
            if (lb != std::string::npos) o = o.substr(0, lb) + ",\"plan_data\":" + emb + "}";
        }
    }
    std::string result = "[";
    for (size_t i = 0; i < objs.size(); ++i) { if (i) result += ","; result += objs[i]; }
    result += "]";
    return result;
}

// Action Implementations
void Suscripcion::Crear() {
...
Task handle_client(int client_fd, struct sockaddr_in address, int addrlen) {
    ...
    if (!req.empty()) {
        ...
        try {
            ...
            if (req.rfind("GET /admin ", 0) == 0 || req.find("GET /admin?") != std::string::npos) {
            ...
            else if (req.find("GET /api/admin/Plan") != std::string::npos) {
                std::string json = Plan::getAllAsJSON(req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("POST /api/admin/Plan") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                ...
                {
                    std::string val = getJSONVal(body, "titulo");
                    if (!val.empty()) instance.titulo = val;
                }
                instance.save();
            ...
            else if (req.find("DELETE /api/admin/Plan") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                std::string valToDelete = getJSONVal(body, "id");
                Plan::deleteRecord("id", valToDelete);
                std::string msg = "{\"status\":\"success\"}";
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("GET /api/admin/Suscripcion") != std::string::npos) {
                std::string json = Suscripcion::getAllAsJSON(req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
                     << "Access-Control-Allow-Origin: *\r\n"
            ...
            else if (req.find("POST /api/admin/Suscripcion") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                ...
                {
                    std::string val = getJSONVal(body, "plan");
                    if (!val.empty()) instance.plan = safeStoi(val);
                }
                instance.save();
                std::string msg = "{\"status\":\"success\"}";
                std::stringstream resp;
            ...
            else if (req.find("DELETE /api/admin/Suscripcion") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                std::string valToDelete = getJSONVal(body, "id");
                Suscripcion::deleteRecord("id", valToDelete);
                std::string msg = "{\"status\":\"success\"}";
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("GET /api/Plan") != std::string::npos) {
                std::string json = Plan::getAllAsJSON(req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("GET /api/Suscripcion") != std::string::npos) {
                std::string json = applyPreloads_Suscripcion(Suscripcion::getAllAsJSON(req), req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
                     << "Access-Control-Allow-Origin: *\r\n"
            ...
            else if (matchDynamicRoute(req, "POST", "/api/subs", pathParams)) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                bool isMultipart = false;
                std::vector<MultipartPart> mpParts;
                ...
                std::cout << "[HTTP Endpoint] Invoked /api/subs -> Running Suscripcion.Crear" << std::endl;
                Suscripcion instance;
                {
                    std::string val = isMultipart ? getMultipartVal(mpParts, "id") : getJSONVal(body, "id");
                    instance.id = safeStoi(val);
                }
                {
                    std::string val = isMultipart ? getMultipartVal(mpParts, "plan") : getJSONVal(body, "plan");
                    instance.plan = safeStoi(val);
                }
                {
                ...
                instance.save();
                instance.Crear();
                broadcast_ws_message("{\"event\": \"action\", \"target\": \"Suscripcion.Crear\"}");
                std::stringstream json;
                json << "{\n"
                     << "  \"status\": \"success\",\n"
                     << "  \"message\": \"Action Suscripcion.Crear executed successfully!\"\n"
                     << "}";
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
