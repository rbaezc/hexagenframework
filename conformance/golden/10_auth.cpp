...
public:
    ...
    void saveJSONL() {
        std::ofstream outfile("db_User.jsonl", std::ios::app);
        if (outfile.is_open()) {
            outfile << "{";
            outfile << "\"email\":";
            outfile << "\"" << email << "\"";
    ...
    static std::string getAllAsJSON_JSONL(const std::string& req = "") {
        std::ifstream infile("db_User.jsonl");
        std::stringstream ss;
        ss << "[";
        int limitVal = -1;
        int offsetVal = 0;
        ...
        std::string filter_email = getQueryParam(req, "email");
        std::string filter_password = getQueryParam(req, "password");
        std::string filter_rol = getQueryParam(req, "rol");
        int matchedCount = 0;
        int skipped = 0;
        if (infile.is_open()) {
            std::string line;
            ...
            while (std::getline(infile, line)) {
                if (line.empty()) continue;
                bool matches = true;
                if (!filter_email.empty()) {
                    if (getJSONVal(line, "email") != filter_email) {
                        matches = false;
                    }
                }
                if (!filter_password.empty()) {
                    if (getJSONVal(line, "password") != filter_password) {
                        matches = false;
                    }
                }
                if (!filter_rol.empty()) {
                    if (getJSONVal(line, "rol") != filter_rol) {
                        matches = false;
                    }
                }
                if (!matches) continue;
    ...
    static void deleteRecord_JSONL(const std::string& key, const std::string& value) {
        ...
        std::ofstream outfile("db_User.jsonl", std::ios::trunc);
        if (outfile.is_open()) {
    ...
    static bool findUser_JSONL(const std::string& emailVal, User& user) {
        std::ifstream infile("db_User.jsonl");
        if (infile.is_open()) {
            std::string line;
            while (std::getline(infile, line)) {
                if (line.empty()) continue;
                if (getJSONVal(line, "email") == emailVal) {
                    user.email = getJSONVal(line, "email");
                    user.password = getJSONVal(line, "password");
                    user.rol = getJSONVal(line, "rol");
                    infile.close();
                    return true;
                }
            }
    ...
    static bool findUser(const std::string& emailVal, User& user) {
        return findUser_JSONL(emailVal, user);
    }
...
public:
    ...
    void saveJSONL() {
        std::ofstream outfile("db_Secreto.jsonl", std::ios::app);
        if (outfile.is_open()) {
            outfile << "{";
            outfile << "\"dato\":";
            outfile << "\"" << dato << "\"";
    ...
    static std::string getAllAsJSON_JSONL(const std::string& req = "") {
        std::ifstream infile("db_Secreto.jsonl");
        std::stringstream ss;
        ss << "[";
        int limitVal = -1;
        int offsetVal = 0;
        ...
        std::string filter_dato = getQueryParam(req, "dato");
        int matchedCount = 0;
        int skipped = 0;
        if (infile.is_open()) {
            std::string line;
            ...
            while (std::getline(infile, line)) {
                if (line.empty()) continue;
                bool matches = true;
                if (!filter_dato.empty()) {
                    if (getJSONVal(line, "dato") != filter_dato) {
                        matches = false;
                    }
                }
                if (!matches) continue;
    ...
    static void deleteRecord_JSONL(const std::string& key, const std::string& value) {
        ...
        std::ofstream outfile("db_Secreto.jsonl", std::ios::trunc);
        if (outfile.is_open()) {
            for (const auto& l : lines) {
                outfile << l << "\n";
            }
...
const slices = {
    "User": [
        { "name": "email", "type": "std::string" },
        { "name": "password", "type": "std::string" },
        { "name": "rol", "type": "std::string" }
    ],
    "Secreto": [
        { "name": "dato", "type": "std::string" }
    ],
};
...
Task handle_client(int client_fd, struct sockaddr_in address, int addrlen) {
    ...
    if (!req.empty()) {
        ...
        try {
            ...
            if (req.rfind("GET /admin ", 0) == 0 || req.find("GET /admin?") != std::string::npos) {
            ...
            else if (req.find("GET /api/admin/User") != std::string::npos) {
                std::string json = User::getAllAsJSON(req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("POST /api/admin/User") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                User instance;
                {
                    std::string val = getJSONVal(body, "email");
                    if (!val.empty()) instance.email = val;
                }
                {
                    std::string val = getJSONVal(body, "password");
                    if (!val.empty()) instance.password = val;
                }
                {
                    std::string val = getJSONVal(body, "rol");
                    if (!val.empty()) instance.rol = val;
                }
                instance.save();
            ...
            else if (req.find("DELETE /api/admin/User") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                std::string valToDelete = getJSONVal(body, "email");
                User::deleteRecord("email", valToDelete);
                std::string msg = "{\"status\":\"success\"}";
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("GET /api/admin/Secreto") != std::string::npos) {
                std::string json = Secreto::getAllAsJSON(req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
                     << "Access-Control-Allow-Origin: *\r\n"
            ...
            else if (req.find("POST /api/admin/Secreto") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                Secreto instance;
                {
                    std::string val = getJSONVal(body, "dato");
                    if (!val.empty()) instance.dato = val;
                }
                instance.save();
                std::string msg = "{\"status\":\"success\"}";
                std::stringstream resp;
            ...
            else if (req.find("DELETE /api/admin/Secreto") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                std::string valToDelete = getJSONVal(body, "dato");
                Secreto::deleteRecord("dato", valToDelete);
                std::string msg = "{\"status\":\"success\"}";
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("GET /api/User") != std::string::npos) {
                std::string json = User::getAllAsJSON(req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("GET /api/Secreto") != std::string::npos) {
                std::string json = Secreto::getAllAsJSON(req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("POST /api/signup") != std::string::npos || req.find("POST /api/User/signup") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                std::string emailVal = getJSONVal(body, "email");
                std::string passVal = getJSONVal(body, "password");
                if (emailVal.empty() || passVal.empty()) {
                    std::string msg = "{\"status\":\"error\",\"message\":\"Missing email or password\"}";
                    std::stringstream resp;
                    ...
                    User existing;
                    if (User::findUser(emailVal, existing)) {
                        std::string msg = "{\"status\":\"error\",\"message\":\"User already exists\"}";
                        std::stringstream resp;
                        resp << "HTTP/1.1 400 Bad Request\r\n"
                             << "Content-Type: application/json\r\n"
                             << "Access-Control-Allow-Origin: *\r\n"
                        ...
                        User user;
                        user.email = emailVal;
                        user.password = hashPassword(passVal);
                        user.rol = getJSONVal(body, "rol");
                        user.save();
                        std::string msg = "{\"status\":\"success\",\"message\":\"User registered successfully\"}";
                        std::stringstream resp;
            ...
            else if (req.find("POST /api/login") != std::string::npos || req.find("POST /api/User/login") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                std::string emailVal = getJSONVal(body, "email");
                std::string passVal = getJSONVal(body, "password");
                User user;
                if (emailVal.empty() || passVal.empty() || !User::findUser(emailVal, user) || !verifyPassword(passVal, user.password)) {
                    std::string msg = "{\"status\":\"error\",\"message\":\"Invalid email or password\"}";
                    std::stringstream resp;
                    ...
                    payload << "}";
                    std::string token = generateSessionToken(payload.str());
                    std::stringstream msg;
                    msg << "{\"status\":\"success\",\"token\":\"" << token << "\"}";
                    std::stringstream resp;
                    resp << "HTTP/1.1 200 OK\r\n"
                         << "Content-Type: application/json\r\n"
                         << "Access-Control-Allow-Origin: *\r\n"
            ...
            else if (matchDynamicRoute(req, "POST", "/api/secreto", pathParams)) {
                ...
                std::cout << "[HTTP Endpoint] Invoked /api/secreto -> Running Secreto.Crear" << std::endl;
                Secreto instance;
                {
                    std::string val = isMultipart ? getMultipartVal(mpParts, "dato") : getJSONVal(body, "dato");
                    instance.dato = val;
                }
                {
                ...
                instance.save();
                instance.Crear();
                broadcast_ws_message("{\"event\": \"action\", \"target\": \"Secreto.Crear\"}");
                std::stringstream json;
                json << "{\n"
                     << "  \"status\": \"success\",\n"
                     << "  \"message\": \"Action Secreto.Crear executed successfully!\"\n"
                     << "}";
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
