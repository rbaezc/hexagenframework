...
public:
    ...
    void saveJSONL() {
        static const std::vector<ColumnSpec> _cols = {{"email", 's'}, {"password", 's'}, {"edad", 'i'}};
        std::vector<std::string> _vals = {email, password, std::to_string(edad)};
        getStorage()->insert("Cuenta", _cols, _vals);
    }

    static std::string getAllAsJSON_JSONL(const std::string& req = "") {
        static const std::vector<ColumnSpec> _cols = {{"email", 's'}, {"password", 's'}, {"edad", 'i'}};
        return getStorage()->selectAllJson("Cuenta", _cols, req);
    }

    static void deleteRecord_JSONL(const std::string& key, const std::string& value) {
        getStorage()->deleteWhere("Cuenta", key, value);
    }
