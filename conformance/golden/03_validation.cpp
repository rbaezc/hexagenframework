...
public:
    ...
    std::map<std::string, std::string> validateChangeset() {
        ...
        if (errors.find("password") == errors.end() && (password.size() < 8 || password.size() > 64)) errors["password"] = "must be between 8 and 64 characters";
        if (errors.find("email") == errors.end() && !isValidEmail(email)) errors["email"] = "has invalid format";
        if (edad < 18) errors["edad"] = "must be at least 18";
        if (edad > 120) errors["edad"] = "must be at most 120";
        return errors;
    }

    void saveJSONL() {
        std::ofstream outfile("db_Cuenta.jsonl", std::ios::app);
        if (outfile.is_open()) {
            outfile << "{";
            outfile << "\"email\":";
            outfile << "\"" << email << "\"";
    ...
    static std::string getAllAsJSON_JSONL(const std::string& req = "") {
        std::ifstream infile("db_Cuenta.jsonl");
        std::stringstream ss;
        ss << "[";
        int limitVal = -1;
        int offsetVal = 0;
        ...
        std::string filter_edad = getQueryParam(req, "edad");
        int matchedCount = 0;
        int skipped = 0;
        if (infile.is_open()) {
            std::string line;
            ...
            while (std::getline(infile, line)) {
                ...
                if (!filter_edad.empty()) {
                    if (getJSONVal(line, "edad") != filter_edad) {
                        matches = false;
                    }
                }
                if (!matches) continue;
    ...
    static void deleteRecord_JSONL(const std::string& key, const std::string& value) {
        ...
        std::ofstream outfile("db_Cuenta.jsonl", std::ios::trunc);
        if (outfile.is_open()) {
...
Task handle_client(int client_fd, struct sockaddr_in address, int addrlen) {
    ...
    if (!req.empty()) {
        ...
        try {
            ...
            if (req.rfind("GET /admin ", 0) == 0 || req.find("GET /admin?") != std::string::npos) {
            ...
            else if (req.find("GET /api/admin/Cuenta") != std::string::npos) {
                std::string json = Cuenta::getAllAsJSON(req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("POST /api/admin/Cuenta") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                ...
                {
                    std::string val = getJSONVal(body, "edad");
                    if (!val.empty()) instance.edad = safeStoi(val);
                }
                instance.save();
            ...
            else if (req.find("DELETE /api/admin/Cuenta") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                std::string valToDelete = getJSONVal(body, "email");
                Cuenta::deleteRecord("email", valToDelete);
                std::string msg = "{\"status\":\"success\"}";
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("GET /api/Cuenta") != std::string::npos) {
                std::string json = Cuenta::getAllAsJSON(req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (matchDynamicRoute(req, "POST", "/api/cuentas", pathParams)) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                bool isMultipart = false;
                std::vector<MultipartPart> mpParts;
                ...
                std::cout << "[HTTP Endpoint] Invoked /api/cuentas -> Running Cuenta.Crear" << std::endl;
                Cuenta instance;
                {
                    std::string val = isMultipart ? getMultipartVal(mpParts, "email") : getJSONVal(body, "email");
                    instance.email = val;
                }
                {
                    std::string val = isMultipart ? getMultipartVal(mpParts, "password") : getJSONVal(body, "password");
                    instance.password = val;
                }
                {
                    std::string val = isMultipart ? getMultipartVal(mpParts, "edad") : getJSONVal(body, "edad");
                    instance.edad = safeStoi(val);
                }
                {
                ...
                instance.save();
                instance.Crear();
                broadcast_ws_message("{\"event\": \"action\", \"target\": \"Cuenta.Crear\"}");
                std::stringstream json;
                json << "{\n"
                     << "  \"status\": \"success\",\n"
                     << "  \"message\": \"Action Cuenta.Crear executed successfully!\"\n"
                     << "}";
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
