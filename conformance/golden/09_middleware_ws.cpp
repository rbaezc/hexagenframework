...
public:
    ...
    void saveJSONL() {
        std::ofstream outfile("db_Mensaje.jsonl", std::ios::app);
        if (outfile.is_open()) {
            outfile << "{";
            outfile << "\"texto\":";
            outfile << "\"" << texto << "\"";
    ...
    static std::string getAllAsJSON_JSONL(const std::string& req = "") {
        std::ifstream infile("db_Mensaje.jsonl");
        std::stringstream ss;
        ss << "[";
        int limitVal = -1;
        int offsetVal = 0;
        ...
        std::string filter_texto = getQueryParam(req, "texto");
        int matchedCount = 0;
        int skipped = 0;
        if (infile.is_open()) {
            std::string line;
            ...
            while (std::getline(infile, line)) {
                if (line.empty()) continue;
                bool matches = true;
                if (!filter_texto.empty()) {
                    if (getJSONVal(line, "texto") != filter_texto) {
                        matches = false;
                    }
                }
                if (!matches) continue;
    ...
    static void deleteRecord_JSONL(const std::string& key, const std::string& value) {
        ...
        std::ofstream outfile("db_Mensaje.jsonl", std::ios::trunc);
        if (outfile.is_open()) {
...
Task handle_client(int client_fd, struct sockaddr_in address, int addrlen) {
    ...
    if (!req.empty()) {
        ...
        try {
            {
                size_t _lf = req.find('\n');
                std::string _line = (_lf == std::string::npos) ? req : req.substr(0, _lf);
                while (!_line.empty() && (_line.back() == '\r' || _line.back() == '\n')) _line.pop_back();
                std::time_t _t = std::time(nullptr);
                char _tb[32]; std::strftime(_tb, sizeof(_tb), "%Y-%m-%d %H:%M:%S", std::localtime(&_t));
                std::cout << "[" << _tb << "] " << inet_ntoa(address.sin_addr) << " " << _line << std::endl;
            }
            ...
            bool is_api_req = (req.find("/api/") != std::string::npos);
            if (!is_api_req) {
                {
                    size_t pos = req.find(" " "/");
                    if (pos != std::string::npos) {
                        char next_char = req[pos + 2];
                        if (next_char == ' ' || next_char == '?' || next_char == '/') is_api_req = true;
                    }
                }
                {
                    size_t pos = req.find(" " "/api/msg");
                    if (pos != std::string::npos) {
                        char next_char = req[pos + 9];
                        if (next_char == ' ' || next_char == '?' || next_char == '/') is_api_req = true;
                    }
                }
            }
            if (is_api_req) {
                ...
                if (!check_rate_limit(client_ip, 100, 60)) {
                    std::string msg = "{\"status\":\"error\",\"message\":\"Rate limit exceeded. Try again later.\"}";
                    std::stringstream resp;
                    ...
                    resp << "Content-Length: " << msg.length() << "\r\n\r\n"
                         << msg;
                    send(client_fd, resp.str().c_str(), resp.str().length(), 0);
                    close(client_fd);
                    co_return;
            ...
            if (upgradeHeader == "websocket") {
                ...
                if (req.find("GET /ws ") != std::string::npos) {
                    std::string wsKey = getHeaderValue(req, "Sec-WebSocket-Key");
                    if (!wsKey.empty()) {
                        std::string acceptKey = getWebSocketAcceptKey(wsKey);
                        std::stringstream handshake;
                        ...
                        std::thread([client_fd]() {
                            ...
                            while (readWebSocketFrame(client_fd, msg)) {
                                if (!msg.empty()) {
                                    ...
                                    Mensaje instance;
                                    {
                                        std::string val = getJSONVal(msg, "texto");
                                        if (!val.empty()) instance.texto = val;
                                    }
                                    instance.save();
                                    instance.Enviar();
                                    broadcast_ws_message(msg, client_fd);
            ...
            if (req.rfind("GET /admin ", 0) == 0 || req.find("GET /admin?") != std::string::npos) {
            ...
            else if (req.find("GET /api/admin/Mensaje") != std::string::npos) {
                std::string json = Mensaje::getAllAsJSON(req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
                     << "Access-Control-Allow-Origin: *\r\n"
            ...
            else if (req.find("POST /api/admin/Mensaje") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                Mensaje instance;
                {
                    std::string val = getJSONVal(body, "texto");
                    if (!val.empty()) instance.texto = val;
                }
                instance.save();
            ...
            else if (req.find("DELETE /api/admin/Mensaje") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                std::string valToDelete = getJSONVal(body, "texto");
                Mensaje::deleteRecord("texto", valToDelete);
                std::string msg = "{\"status\":\"success\"}";
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("GET /api/Mensaje") != std::string::npos) {
                std::string json = Mensaje::getAllAsJSON(req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (matchDynamicRoute(req, "POST", "/api/msg", pathParams)) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                bool isMultipart = false;
                std::vector<MultipartPart> mpParts;
                ...
                std::cout << "[HTTP Endpoint] Invoked /api/msg -> Running Mensaje.Enviar" << std::endl;
                Mensaje instance;
                {
                    std::string val = isMultipart ? getMultipartVal(mpParts, "texto") : getJSONVal(body, "texto");
                    instance.texto = val;
                }
                {
                ...
                instance.save();
                instance.Enviar();
                broadcast_ws_message("{\"event\": \"action\", \"target\": \"Mensaje.Enviar\"}");
                std::stringstream json;
                json << "{\n"
                     << "  \"status\": \"success\",\n"
                     << "  \"message\": \"Action Mensaje.Enviar executed successfully!\"\n"
                     << "}";
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (matchDynamicRoute(req, "WEBSOCKET", "/ws", pathParams)) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                bool isMultipart = false;
                std::vector<MultipartPart> mpParts;
                ...
                std::cout << "[HTTP Endpoint] Invoked /ws -> Running Mensaje.Enviar" << std::endl;
                Mensaje instance;
                {
                    std::string val = isMultipart ? getMultipartVal(mpParts, "texto") : getJSONVal(body, "texto");
                    instance.texto = val;
