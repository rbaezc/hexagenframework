...
public:
    ...
    void saveJSONL() {
        static const std::vector<ColumnSpec> _cols = {{"texto", 's'}};
        std::vector<std::string> _vals = {texto};
        getStorage()->insert("Mensaje", _cols, _vals);
    }

    static std::string getAllAsJSON_JSONL(const std::string& req = "") {
        static const std::vector<ColumnSpec> _cols = {{"texto", 's'}};
        return getStorage()->selectAllJson("Mensaje", _cols, req);
    }

    static void deleteRecord_JSONL(const std::string& key, const std::string& value) {
        getStorage()->deleteWhere("Mensaje", key, value);
    }
