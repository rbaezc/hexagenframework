...
public:
    ...
    void saveJSONL() {
        static const std::vector<ColumnSpec> _cols = {{"nombre", 's'}, {"precio", 'i'}};
        std::vector<std::string> _vals = {nombre, std::to_string(precio)};
        getStorage()->insert("Item", _cols, _vals);
    }

    static std::string getAllAsJSON_JSONL(const std::string& req = "") {
        static const std::vector<ColumnSpec> _cols = {{"nombre", 's'}, {"precio", 'i'}};
        return getStorage()->selectAllJson("Item", _cols, req);
    }

    static void deleteRecord_JSONL(const std::string& key, const std::string& value) {
        getStorage()->deleteWhere("Item", key, value);
    }
