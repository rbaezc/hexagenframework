...
void initDatabase() {
    ...
    if (db) {
        {
            std::string q = "CREATE TABLE IF NOT EXISTS \"Item\" (id INTEGER PRIMARY KEY AUTOINCREMENT, \"nombre\" TEXT, \"precio\" INTEGER);";
            char* errMsg = nullptr;
...
public:
    ...
    void saveJSONL() {
        std::ofstream outfile("db_Item.jsonl", std::ios::app);
        if (outfile.is_open()) {
            outfile << "{";
            outfile << "\"nombre\":";
            outfile << "\"" << nombre << "\"";
    ...
    static std::string getAllAsJSON_JSONL(const std::string& req = "") {
        std::ifstream infile("db_Item.jsonl");
        std::stringstream ss;
        ss << "[";
        int limitVal = -1;
        int offsetVal = 0;
        ...
        std::string filter_nombre = getQueryParam(req, "nombre");
        std::string filter_precio = getQueryParam(req, "precio");
        int matchedCount = 0;
        int skipped = 0;
        if (infile.is_open()) {
            std::string line;
            ...
            while (std::getline(infile, line)) {
                if (line.empty()) continue;
                bool matches = true;
                if (!filter_nombre.empty()) {
                    if (getJSONVal(line, "nombre") != filter_nombre) {
                        matches = false;
                    }
                }
                if (!filter_precio.empty()) {
                    if (getJSONVal(line, "precio") != filter_precio) {
                        matches = false;
                    }
                }
                if (!matches) continue;
    ...
    static void deleteRecord_JSONL(const std::string& key, const std::string& value) {
        ...
        std::ofstream outfile("db_Item.jsonl", std::ios::trunc);
        if (outfile.is_open()) {
    ...
    static std::string getAllAsJSON(const std::string& req = "") {
        sqlite3* db = getSQLiteConn();
        if (!db) return getAllAsJSON_JSONL(req);
        std::vector<std::pair<std::string, std::string>> filters;
        {
            std::string val = getQueryParam(req, "nombre");
            if (!val.empty()) {
                filters.push_back({"nombre", val});
            }
        }
        {
            std::string val = getQueryParam(req, "precio");
            if (!val.empty()) {
                filters.push_back({"precio", val});
            }
        }
        ...
        if (sqlite3_prepare_v2(db, query.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
            ...
            for (const auto& f : filters) {
                sqlite3_bind_text(stmt, bindIdx++, f.second.c_str(), -1, SQLITE_TRANSIENT);
            }
            if (!limitStr.empty()) {
                sqlite3_bind_int(stmt, bindIdx++, safeStoi(limitStr));
            }
            if (!offsetStr.empty()) {
                sqlite3_bind_int(stmt, bindIdx++, safeStoi(offsetStr));
            }
            bool first = true;
    ...
    static void deleteRecord(const std::string& key, const std::string& value) {
        ...
        std::string query = "DELETE FROM \"Item\" WHERE \"" + key + "\" = ?;";
        sqlite3_stmt* stmt;
        if (sqlite3_prepare_v2(db, query.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
...
Task handle_client(int client_fd, struct sockaddr_in address, int addrlen) {
    ...
    if (!req.empty()) {
        ...
        try {
            ...
            if (req.rfind("GET /admin ", 0) == 0 || req.find("GET /admin?") != std::string::npos) {
            ...
            else if (req.find("GET /api/admin/Item") != std::string::npos) {
                std::string json = Item::getAllAsJSON(req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("POST /api/admin/Item") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                ...
                {
                    std::string val = getJSONVal(body, "precio");
                    if (!val.empty()) instance.precio = safeStoi(val);
                }
                instance.save();
            ...
            else if (req.find("DELETE /api/admin/Item") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                std::string valToDelete = getJSONVal(body, "nombre");
                Item::deleteRecord("nombre", valToDelete);
                std::string msg = "{\"status\":\"success\"}";
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("GET /api/Item") != std::string::npos) {
                std::string json = Item::getAllAsJSON(req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (matchDynamicRoute(req, "POST", "/api/items", pathParams)) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                bool isMultipart = false;
                std::vector<MultipartPart> mpParts;
                ...
                std::cout << "[HTTP Endpoint] Invoked /api/items -> Running Item.Crear" << std::endl;
                Item instance;
                {
                ...
                {
                    std::string val = isMultipart ? getMultipartVal(mpParts, "precio") : getJSONVal(body, "precio");
                    instance.precio = safeStoi(val);
                }
                {
                ...
                instance.save();
                instance.Crear();
                broadcast_ws_message("{\"event\": \"action\", \"target\": \"Item.Crear\"}");
                std::stringstream json;
                json << "{\n"
                     << "  \"status\": \"success\",\n"
                     << "  \"message\": \"Action Item.Crear executed successfully!\"\n"
                     << "}";
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
