...
public:
    ...
    void saveJSONL() {
        std::ofstream outfile("db_Disparador.jsonl", std::ios::app);
        if (outfile.is_open()) {
            outfile << "{";
            outfile << "\"x\":";
            outfile << x;
    ...
    static std::string getAllAsJSON_JSONL(const std::string& req = "") {
        std::ifstream infile("db_Disparador.jsonl");
        std::stringstream ss;
        ss << "[";
        int limitVal = -1;
        int offsetVal = 0;
        ...
        std::string filter_x = getQueryParam(req, "x");
        int matchedCount = 0;
        int skipped = 0;
        if (infile.is_open()) {
            std::string line;
    ...
    static void deleteRecord_JSONL(const std::string& key, const std::string& value) {
        ...
        std::ofstream outfile("db_Disparador.jsonl", std::ios::trunc);
        if (outfile.is_open()) {
