...
public:
    ...
    void saveJSONL() {
        static const std::vector<ColumnSpec> _cols = {{"x", 'i'}};
        std::vector<std::string> _vals = {std::to_string(x)};
        getStorage()->insert("Disparador", _cols, _vals);
    }

    static std::string getAllAsJSON_JSONL(const std::string& req = "") {
        static const std::vector<ColumnSpec> _cols = {{"x", 'i'}};
        return getStorage()->selectAllJson("Disparador", _cols, req);
    }

    static void deleteRecord_JSONL(const std::string& key, const std::string& value) {
        getStorage()->deleteWhere("Disparador", key, value);
    }
...
void Disparador::Lanzar() {
        {
            auto jobInst = std::make_shared<Job_Email>();
            jobInst->to = "a@b.com";
            jobInst->intentos = 2;
            std::stringstream _aj; _aj << "{";
            _aj << "\"to\":";
            _aj << "\"" << jobInst->to << "\"";
            _aj << ",";
            _aj << "\"intentos\":";
            _aj << jobInst->intentos;
            _aj << "}";
            enqueue_persisted_job("Email", _aj.str(), [jobInst]() { jobInst->Run(); });
        }
}

void Job_Email::Run() {
        std::cout << "email " << to << std::endl;
}

void dispatch_job(const std::string& name, const std::string& argsJson) {
    (void)argsJson;
    if (name == "Email") {
        auto inst = std::make_shared<Job_Email>();
        inst->to = getJSONVal(argsJson, "to");
        inst->intentos = safeStoi(getJSONVal(argsJson, "intentos"));
        inst->Run();
    }
}
...
Task handle_client(int client_fd, struct sockaddr_in address, int addrlen) {
    ...
    if (!req.empty()) {
        ...
        try {
            ...
            if (req.rfind("GET /admin ", 0) == 0 || req.find("GET /admin?") != std::string::npos) {
            ...
            else if (req.find("GET /api/admin/Disparador") != std::string::npos) {
                std::string json = Disparador::getAllAsJSON(req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("POST /api/admin/Disparador") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                Disparador instance;
                {
                    std::string val = getJSONVal(body, "x");
                    if (!val.empty()) instance.x = safeStoi(val);
                }
                instance.save();
            ...
            else if (req.find("DELETE /api/admin/Disparador") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                std::string valToDelete = getJSONVal(body, "x");
                Disparador::deleteRecord("x", valToDelete);
                std::string msg = "{\"status\":\"success\"}";
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("GET /api/Disparador") != std::string::npos) {
                std::string json = Disparador::getAllAsJSON(req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (matchDynamicRoute(req, "POST", "/api/run", pathParams)) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                bool isMultipart = false;
                std::vector<MultipartPart> mpParts;
                ...
                std::cout << "[HTTP Endpoint] Invoked /api/run -> Running Disparador.Lanzar" << std::endl;
                Disparador instance;
                {
                    std::string val = isMultipart ? getMultipartVal(mpParts, "x") : getJSONVal(body, "x");
                    instance.x = safeStoi(val);
                }
                {
                ...
                instance.save();
                instance.Lanzar();
                broadcast_ws_message("{\"event\": \"action\", \"target\": \"Disparador.Lanzar\"}");
                std::stringstream json;
                json << "{\n"
                     << "  \"status\": \"success\",\n"
                     << "  \"message\": \"Action Disparador.Lanzar executed successfully!\"\n"
                     << "}";
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
