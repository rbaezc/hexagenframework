...
public:
    ...
    void saveJSONL() {
        static const std::vector<ColumnSpec> _cols = {{"titulo", 's'}, {"hecha", 'b'}};
        std::vector<std::string> _vals = {titulo, (hecha ? std::string("true") : std::string("false"))};
        getStorage()->insert("Tarea", _cols, _vals);
    }

    static std::string getAllAsJSON_JSONL(const std::string& req = "") {
        static const std::vector<ColumnSpec> _cols = {{"titulo", 's'}, {"hecha", 'b'}};
        return getStorage()->selectAllJson("Tarea", _cols, req);
    }

    static void deleteRecord_JSONL(const std::string& key, const std::string& value) {
        getStorage()->deleteWhere("Tarea", key, value);
    }
