...
public:
    ...
    void saveJSONL() {
        std::ofstream outfile("db_Tarea.jsonl", std::ios::app);
        if (outfile.is_open()) {
            outfile << "{";
            outfile << "\"titulo\":";
            outfile << "\"" << titulo << "\"";
    ...
    static std::string getAllAsJSON_JSONL(const std::string& req = "") {
        std::ifstream infile("db_Tarea.jsonl");
        std::stringstream ss;
        ss << "[";
        int limitVal = -1;
        int offsetVal = 0;
        ...
        std::string filter_hecha = getQueryParam(req, "hecha");
        int matchedCount = 0;
        int skipped = 0;
        if (infile.is_open()) {
            std::string line;
            ...
            while (std::getline(infile, line)) {
                ...
                if (!filter_hecha.empty()) {
                    if (getJSONVal(line, "hecha") != filter_hecha) {
                        matches = false;
                    }
                }
                if (!matches) continue;
    ...
    static void deleteRecord_JSONL(const std::string& key, const std::string& value) {
        ...
        std::ofstream outfile("db_Tarea.jsonl", std::ios::trunc);
        if (outfile.is_open()) {
...
void Tarea::Crear() {
        std::cout << "tarea creada" << std::endl;
}
...
Task handle_client(int client_fd, struct sockaddr_in address, int addrlen) {
    ...
    if (!req.empty()) {
        ...
        try {
            ...
            if (req.rfind("GET /admin ", 0) == 0 || req.find("GET /admin?") != std::string::npos) {
            ...
            else if (req.find("GET /api/admin/Tarea") != std::string::npos) {
                std::string json = Tarea::getAllAsJSON(req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("POST /api/admin/Tarea") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                ...
                {
                    std::string val = getJSONVal(body, "hecha");
                    if (!val.empty()) instance.hecha = (val == "true" || val == "1");
                }
                instance.save();
            ...
            else if (req.find("DELETE /api/admin/Tarea") != std::string::npos) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                std::string valToDelete = getJSONVal(body, "titulo");
                Tarea::deleteRecord("titulo", valToDelete);
                std::string msg = "{\"status\":\"success\"}";
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (req.find("GET /api/Tarea") != std::string::npos) {
                std::string json = Tarea::getAllAsJSON(req);
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
            ...
            else if (matchDynamicRoute(req, "POST", "/api/tareas", pathParams)) {
                size_t bodyPos = req.find("\r\n\r\n");
                std::string body = (bodyPos != std::string::npos) ? req.substr(bodyPos + 4) : "";
                bool isMultipart = false;
                std::vector<MultipartPart> mpParts;
                ...
                std::cout << "[HTTP Endpoint] Invoked /api/tareas -> Running Tarea.Crear" << std::endl;
                Tarea instance;
                {
                    std::string val = isMultipart ? getMultipartVal(mpParts, "titulo") : getJSONVal(body, "titulo");
                    instance.titulo = val;
                }
                {
                    std::string val = isMultipart ? getMultipartVal(mpParts, "hecha") : getJSONVal(body, "hecha");
                    instance.hecha = (val == "true" || val == "1");
                }
                {
                ...
                instance.save();
                instance.Crear();
                broadcast_ws_message("{\"event\": \"action\", \"target\": \"Tarea.Crear\"}");
                std::stringstream json;
                json << "{\n"
                     << "  \"status\": \"success\",\n"
                     << "  \"message\": \"Action Tarea.Crear executed successfully!\"\n"
                     << "}";
                std::stringstream resp;
                resp << "HTTP/1.1 200 OK\r\n"
                     << "Content-Type: application/json\r\n"
